...
async function find({ collection, query, options, project, limit, skip, populate, sort }) {
  const result = await dbModels[collection].find(query, project, options).limit(limit).sort(sort).skip(skip).populate(populate).lean()
  return result
}

async function findAll({ collection, query }) {
  const result = await dbModels[collection].find(query).lean()

  return result
}

async function findOne({ collection, query, project, populate, sort }) {
  const result = await dbModels[collection].findOne(query, project).sort(sort).populate(populate).lean()
  return result
}

async function insertOne({ collection, document, options }) {
  const result = await dbModels[collection].create(document, options)
  return result
}
...
async function findByIdAndUpdate({ collection, id, update, options }) {
  console.log(" id ", id);
  const result = await dbModels[collection].findByIdAndUpdate({ _id: ObjectId(id) }, update, options)
  return result
}
